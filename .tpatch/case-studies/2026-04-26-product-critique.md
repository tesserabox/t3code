...
1. **Intent tracking** — `spec.md`, `analysis.md`, `exploration.md` capture _why_ a change exists, not just _what_ changed. Git commits have messages, but they're retrospective. tpatch's phases force you to articulate intent _before_ coding.
...
The "up to date" framing undersells it. Keeping forks up to date is `git merge upstream/main`. tpatch's value is knowing _which of your 18 custom features survived the merge_ and _which need re-implementation_.
