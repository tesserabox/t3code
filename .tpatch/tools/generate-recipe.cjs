...
function getFiles(filter) {
  ...
  const raw = git(
    `git diff --diff-filter=${filter} --name-only ${fromRef}..${toRef} ${fileScope}`,
  ).trim();
...
function parseHunks(diffText, beforeContent) {
  ...
  for (const hunk of hunks) {
    ...
    for (const line of lines) {
      ...
      if (line[0] === "-") {
        search.push(line.substring(1));
        hasChange = true;
      } else if (line[0] === "+") {
        replace.push(line.substring(1));
        hasChange = true;
      } else if (line[0] === " ") {
        search.push(line.substring(1));
        replace.push(line.substring(1));
...
console.log(
  `${slug}: ${operations.length} ops (${writes} write, ${replaces} replace), ${errors} errors`,
);
